...
def siamese_train():
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % save_every == 0:
            torch.save(model.state_dict(), f"siamese/siamese_model_epoch_{epoch}.pth")
            torch.save(optimizer.state_dict(), f"siamese/siamese_optimizer_epoch_{epoch}.pth")
            print(f"Saved model and optimizer state for epoch {epoch}")

    torch.save(model.state_dict(), "siamese/siamese_model_final.pth")
    torch.save(optimizer.state_dict(), "siamese/siamese_optimizer_final.pth")
